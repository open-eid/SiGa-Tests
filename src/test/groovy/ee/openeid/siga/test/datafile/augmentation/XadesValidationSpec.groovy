...
class XadesValidationSpec extends GenericSpecification {
    ...
    @Story("Container with LT signatures is extended to LTA")
    ...
    @Story("Container that cannot be extended to LTA is wrapped into timestamped ASiC-S")
    def "Augmenting #description wraps container into timestamped ASiC-S"() {
        given: "upload container that cannot be extended to LTA and make a copy of it"
        datafile.uploadContainer(flow, RequestData.uploadDatafileRequestBodyFromFile(containerName))
        byte[] containerBeforeAugmentation = Base64.decoder.decode(datafile.getContainer(flow).path("container").toString())
        ...
        // TODO: SIGA-897: once upload preserves the uploaded bytes, compare against Utils.readFileFromResources(containerName)
        Arrays.equals(containerBeforeAugmentation,
